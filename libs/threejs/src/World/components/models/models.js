...
async function loadModels() {
  ...
  const [tableData, chairData, doorData, coffee_cupData, bellData, light_switchData] = await Promise.all([
  ...
  bell.rotation.z = MathUtils.degToRad(69)
  bell.name = 'bell'

  // light switch
  const light_switch = setupModel(light_switchData)
  light_switch.position.set(-3.95, 2.25, -7.4)
  light_switch.scale.set(0.005, 0.005, 0.005)
  light_switch.rotation.z = MathUtils.degToRad(90)
  light_switch.rotation.y = MathUtils.degToRad(5)
  light_switch.rotation.x = MathUtils.degToRad(270)
  light_switch.name = 'light_switch'
  

  return { chair1, chair2, chair3, chair4, chair5, chair6, chair7, chair8, table, door, coffee_cup, bell, light_switch }
}
